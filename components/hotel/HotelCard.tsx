...
import { Dumbbell, MapIcon, Waves } from "lucide-react";
    ...
    >
      <div className="flex gap-2 bg-background/50 border border-primary/10 rounded-lg ">
        ...
        <div className="flex-1 flex flex-col justify-between h-[210px] gap-1 p-1 py-2 text-sm">
          ...
          <div className="text-primary/90">
            <AmenityItem>
              <MapIcon className="s-4" /> {country?.name}, {hotel.city}
            </AmenityItem>
            {hotel.swimmingPool && (
              <AmenityItem>
                <Waves className="s-4" />
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1">
              {hotel?.rooms[0]?.roomPrice && (
                <>
                  <div className="font-semibold text-based">
                    `${hotel?.rooms[0]?.roomPrice}`
                  </div>
